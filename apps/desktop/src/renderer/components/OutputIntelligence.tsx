...
export default function OutputIntelligence({
  ...
  async function askProject(): Promise<void> {
    ...
    try {
      const nextAnswer = await invoke<RagAnswer>("rag:ask", {
        projectId: project.id,
        question,
        limit: 8,
      });
  ...
  async function copySelectedEntryContext(): Promise<void> {
    ...
    const context = renderEntryContext(selectedEntry);
                      ...
                      primary={
                        <Stack spacing={0.75}>
                          <Typography variant="body2" sx={{ fontWeight: 700 }} noWrap>
                            {isSearchResult(item) ? item.entryName : item.name}
                          </Typography>
        ...
        >
          <Stack direction="row" alignItems="center" spacing={1} sx={{ p: 1.5 }}>
            <Typography variant="subtitle1" sx={{ flex: 1, fontWeight: 700 }} noWrap>
              {selectedEntry?.name ?? "Project memory"}
            </Typography>
            ...
            <Tooltip title="Copy entry context" arrow>
              <span>
                <IconButton
                  ...
                  onClick={() => void copySelectedEntryContext()}
                >
                  <ContentCopyIcon fontSize="small" />
                </IconButton>
              </span>
          ...
          <Box sx={{ minHeight: 0, overflow: "auto", p: 2 }}>
            ...
            {selectedEntry ? (
                ...
                <Box>
                  ...
                  {selectedEntry.changedFiles.length > 0 ? (
                    <Stack spacing={0.5} sx={{ mt: 0.75 }}>
                      {selectedEntry.changedFiles.map((filePath) => (
              ...
              <Typography variant="body2" color="text.secondary">
                Select an entry or ask a project-memory question.
              </Typography>
            )}
          </Box>
        </Paper>
